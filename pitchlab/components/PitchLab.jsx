...
export default function ZoePitcher() {
  ...
  const [motionThreshold, setMotionThreshold] = useState(3);
    ...
    <div style={{ minHeight: "100vh", background: "#f5f7fa", color: "#1e293b", fontFamily: "'Barlow Condensed', 'Impact', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tab:not(.active) { color: #2a3a5a; border-bottom: 2px solid transparent; }
        .tab:hover:not(.active) { color: #4a6a8a; }
        ...
        <div style={{ position: "relative", background: "#f0f4f8", height: 200, overflow: "hidden" }}>
          ...
          {lastPitch && (
            <div className="pop" key={pitches.length} style={{
              ...
              {lastPitch.zone && <span style={{ fontSize: 11, color: "#888", marginLeft: 8, fontFamily: "monospace" }}>
                ZONE {lastPitch.zone}
              </span>}
            </div>
          )}
          ...
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 2, background: "#e2e8f0" }}>
            <div style={{ height: "100%", width: `${Math.min(motionLevel * 3, 100)}%`, background: motionLevel >= motionThreshold ? "#0284c7" : "#94a3b8", transition: "width 0.3s" }} />
          </div>
        </div>
        ...
        {tab === "setup" && (
          <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "#ffffff", borderRadius: 10, padding: "14px", border: "1px solid #0e1628" }}>
              ...
              {[
                { id: "side", label: "Side View — 3rd Base Line ✓ RECOMMENDED", desc: "Set tripod 10–15 feet back from home plate along the 3rd base line. You can see Zoe's full delivery, the ball in flight, AND home plate. Best for mechanics. Practical at any field." },
                { id: "behind", label: "Behind Home Plate (Elevated)", desc: "Camera mounted HIGH on the backstop fence looking toward the pitcher — not on a tripod behind the catcher. Best for zone tracking but requires fence mounting. Not practical at most youth fields." },
              ].map(opt => (
            ...
            <div style={{ background: "#ffffff", borderRadius: 10, padding: "14px", border: "1px solid #e2e8f0" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                <span style={{ fontSize: 11, color: "#94a3b8", fontFamily: "monospace" }}>LOW</span>
                <input type="range" min="1" max="15" value={motionThreshold}
                  onChange={e => setMotionThreshold(Number(e.target.value))}
                  style={{ flex: 1, accentColor: "#0284c7" }} />
                <span style={{ fontSize: 11, color: "#94a3b8", fontFamily: "monospace" }}>HIGH</span>
              </div>
              <div style={{ textAlign: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 20, color: "#0284c7", fontFamily: "monospace" }}>{motionThreshold}%</span>
                <span style={{ fontSize: 10, color: "#94a3b8", fontFamily: "monospace", marginLeft: 6 }}>pixel change threshold</span>
              </div>
              <div style={{ fontSize: 10, color: "#64748b", fontFamily: "monospace", lineHeight: 1.6 }}>
                {motionThreshold <= 3 && "🟢 Very sensitive — best for tripod behind plate. May catch extra frames between pitches."}
                {motionThreshold > 3 && motionThreshold <= 7 && "🟡 Balanced — good starting point. Adjust if pitches are being missed."}
                {motionThreshold > 7 && "🔴 Less sensitive — only fires on large movement. Use if getting too many false triggers."}
              </div>
            </div>

            <div style={{ background: "#fff8ed", borderRadius: 10, padding: "14px", border: "1px solid #fed7aa" }}>
              <div style={{ fontSize: 9, color: "#9a3412", fontFamily: "monospace", letterSpacing: 2, marginBottom: 8 }}>⚠ TRIPOD REQUIRED</div>
              <p style={{ fontSize: 11, color: "#7c2d12", fontFamily: "monospace", lineHeight: 1.6 }}>
                Do NOT hold the phone while catching. A moving camera kills motion detection — every tiny hand movement looks like motion to the app. Set it on a tripod 3–4 feet behind home plate, slightly elevated, and walk away. You catch normally, the camera watches the full scene without moving.
              </p>
            </div>
          </div>
        )}
